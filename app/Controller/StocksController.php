...
public function index() {
	...
	$this->paginate = array(
			...
			'recursive'=>-1,
			'conditions'=>array('Stock.district_id'=>$users['district_id'])
		);
...
public function stock() {
    ...
    //$districtName = $this->District->find('first',array('recursive'=>-1,'conditions'=>array('District.id'=>$this->params['pass'][0]))); 
    $districtList = $this->District->find('list',array('recursive'=>-1,'fields'=>array('namebn'))); 
    $this->set(compact('categories','districtName','districtList'));
			...
			foreach ($products as $stock){
				...
				if(!empty($closebalance['Stockacrchive']['balance'])){
					...
					( 
						...
						FROM purchasedetails WHERE purchasedetails.ddate='".$cdate."' AND district_id=$authdis GROUP BY purchasedetails.product_id 
					)
					 AS p ON pt.id = p.product_id LEFT JOIN 
					( 
						SELECT deliverydetails.product_id,SUM(deliverydetails.quantity) AS dquantity 
						FROM deliverydetails WHERE deliverydetails.ddate='".$cdate."' district_id=$authdis GROUP BY deliverydetails.product_id 
					)
...
public function stockpcheck(){
	$this->layout='ajax';
	 if(!empty($_REQUEST['id'])){
 		$id=$_REQUEST['id'];
 		$disid=$_REQUEST['disid']; 
		$productid = $this->Stock->find('first',array('conditions'=>array('Stock.product_id'=>$id,'Stock.district_id'=>$disid),'recursive'=>-1)); 
...
public function add() {
	...
	$stocks = $this->Stock->find('list',array('fields'=>array('product_id','product_id'),'conditions'=>array('Stock.district_id'=>$users['district_id']))); 
	$this->set(compact('stocks','measures'));
}
...
public function requsition(){
    ...
    public function dashboardrequisitioner(){
	    ...
	    if($role_id==3){
	        ...
	        $totalreqcount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array('Requisition.user_id'=>$id,'Requisition.district_id'=>$district_id))); 
	        $approvedcount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>2,'Requisition.user_id'=>$id,'Requisition.district_id'=>$district_id))); 
	     	$deliveredecount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>4,'Requisition.user_id'=>$id,'Requisition.district_id'=>$district_id))); 
			$rejectedecount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>3,'Requisition.user_id'=>$id,'Requisition.district_id'=>$district_id))); 
			$rejectedecount=$this->Requisitiondetail->find(
				...
				array(
					...
					'conditions'=>array('Requisitiondetail.status'=>3,'Requisition.user_id'=>$id,'Requisitiondetail.district_id'=>$district_id)
				)
			);

			$pendingcount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>1,'Requisition.user_id'=>$id,'Requisition.district_id'=>$district_id))); 

			$delivery=$this->Delivery->find(
				'count',
				array(
					'conditions'=>array(
					'Requisition.user_id'=>$id,'Requisition.status'=>4,'Requisition.district_id'=>$district_id
					),
            ...
            $totalreqcount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>1,'Requisition.district_id'=>$district_id)));
            $approvedcount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>2,'Requisition.district_id'=>$district_id)));
     		$deliveredecount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>4,'Requisition.district_id'=>$district_id)));
			$rejectedecount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>3,'Requisition.district_id'=>$district_id)));
			$pendingcount=$this->Requisition->find('count',array('fields'=>array('id'),'conditions'=>array(' Requisition.status'=>1,'Requisition.district_id'=>$district_id)));            
        }      
	...
	public function getcategory(){
		...
		if (!empty($_REQUEST['disId'])) {
			$getpassDis=$_REQUEST['disId'];
			$this->set(compact('getpassDis'));
		}	
	}
