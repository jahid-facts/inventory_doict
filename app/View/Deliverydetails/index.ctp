<div class="deliverydetails index">
	<h2><?php echo __('Deliverydetails'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<thead>
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('quantity'); ?></th>
			<th><?php echo $this->Paginator->sort('price'); ?></th>
			<th><?php echo $this->Paginator->sort('measure_id'); ?></th>
			<th><?php echo $this->Paginator->sort('deliveries_id'); ?></th>
			<th><?php echo $this->Paginator->sort('voucher'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php foreach ($deliverydetails as $deliverydetail): ?>
	<tr>
		<td><?php echo h($deliverydetail['Deliverydetail']['id']); ?>&nbsp;</td>
		<td><?php echo h($deliverydetail['Deliverydetail']['quantity']); ?>&nbsp;</td>
		<td><?php echo h($deliverydetail['Deliverydetail']['price']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($deliverydetail['Measure']['name'], array('controller' => 'measures', 'action' => 'view', $deliverydetail['Measure']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($deliverydetail['Deliveries']['id'], array('controller' => 'deliveries', 'action' => 'view', $deliverydetail['Deliveries']['id'])); ?>
		</td>
		<td><?php echo h($deliverydetail['Deliverydetail']['voucher']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $deliverydetail['Deliverydetail']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $deliverydetail['Deliverydetail']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $deliverydetail['Deliverydetail']['id']), array(), __('Are you sure you want to delete # %s?', $deliverydetail['Deliverydetail']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Deliverydetail'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Measures'), array('controller' => 'measures', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Measure'), array('controller' => 'measures', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Deliveries'), array('controller' => 'deliveries', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Deliveries'), array('controller' => 'deliveries', 'action' => 'add')); ?> </li>
	</ul>
</div>
