    ...
    <div class="panel panel-primary">
        <div class="panel-heading">
            <h1 class="panel-title" > 
                <?php echo __('Edit Authorized Role'); ?>
                <span class="add-span">
                    ...
                    foreach ($menudata as $key => $value) {
                        ...
                        foreach ($value as $k => $val) {
                            ...
                            if (isset($selvals[$key][$val]) && $selvals[$key][$val] == $val) {
                                echo $this->Form->input("Role.roles.$key.$val", array('type' => 'checkbox', 'checked' => true, 'value' => $val, 'style' => 'margin-left: 0px;'));
                            } else {
                                echo $this->Form->input("Role.roles.$key.$val", array('type' => 'checkbox', 'value' => $val, 'style' => 'margin-left: 0px;'));
                            }
                            echo '</div>';
                        }
                        echo '</div>';
