...
<table id="example" class="display thla" cellspacing="0" width="100%">
    <thead>
        <tr>
            ...
            <th rowspan="2">Requisition Add</th> 
        </tr>
        <tr>
            <th>Code</th>
            <th>Name</th> 
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <?php if($balance>0){?> 
                    <div id="pi<?php echo $pid;?>" class="add-cart-button btn-group btn btn-primary" onclick="addCart('<?php echo $stock['Product']['id'] ?>', '<?php echo $stock['Product']['name'] ?>', '<?php echo $stock['Measure']['id'] ?>', '<?php echo $balance ?>');"> <i class=" glyphicon glyphicon-shopping-cart"></i> Add to cart
                    </div> 
                <?php }?>
            </td>
        </tr>
