        ...
        <div class="panel-heading">
            <h3 class="panel-title">
                <?php if($currentUser['role_id'] ==2){?>
                    <?php echo __('Pending For Delivery'); ?>
                <?php }?>
                <?php if($currentUser['role_id'] ==1 || $currentUser['role_id'] ==4){?>
                    <?php echo __('Total Requisition Approved'); ?>
                <?php }?>
                <?php if($currentUser['role_id'] ==3){?>
                    <?php echo __('Requisition Approved'); ?>
                <?php }?>
            </h3>
        </div>
                ...
                <div class="table-responsive">
                    <table cellpadding="0" cellspacing="0" class="table table-striped table-bordered table-hover thla" id="dataTables-example">
                        ...
                        <tr>
                            ...
                            <td> <?php echo $requisition['Requisition']['location']; ?> </td> 
                            <td class="actions" style="text-align: center;"> 

                            <?php if($currentUser['role_id'] ==2){?>
                                <?php echo $this->Html->link(__('<span class="fa fa-truck"></span> Delivery '),array('action' => 'delivery', $requisition['Requisition']['id'],'approview'),array('class'=>'btn btn-info btn-sm','escape' =>false,'title'=>'Delivery')); ?>           
                            <?php }?>
                            <?php if($currentUser['role_id'] !=2){?>
                            <?php echo $this->Html->link(__('<i class="fa fa-eye" title="View Requisition"></i>'), array('action' => 'view', $requisition['Requisition']['id'],'approview'),array('escape' =>false)); ?>
                            <?php }?>
                            </td>
