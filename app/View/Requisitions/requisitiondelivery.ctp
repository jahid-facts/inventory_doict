        ...
        <div class="panel-heading">
            <h3 class="panel-title"> 
                <?php if($currentUser['role_id'] ==1 || $currentUser['role_id'] ==2 || $currentUser['role_id'] ==4 ){ 
                   echo "Requisitions Approved";
                }else{
                   echo "Delivered Requisition";
                }?> 
            </h3>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="actions" style="text-align: center;">  
                            <?php if($currentUser['role_id'] ==2){?>
                                <?php echo $this->Html->link(__('<span class="fa fa-location-arrow"></span> Prepared Delivery'),array('action' => 'delivery', $requisition['Requisition']['id'],'deliviewed'),array('class'=>'btn btn-info btn-sm','escape' =>false)); ?>  
                            <?php }?>
                            <?php if($currentUser['role_id'] !=2){?>
                                <?php echo $this->Html->link(__('<i class="fa fa-eye" title="View Delivered Requisition"></i>'), array('controller' => 'deliveries','action' => 'deliveryview', $requisition['Delivery'][0]['id'],'deliviewed'),array('escape' =>false)); ?>
                            <?php }?> 
                        </td> 
                    </tr>
                    <?php $i++;endforeach; ?>
