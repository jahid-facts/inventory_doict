        ...
        <div class="panel-heading">
            <h3 class="panel-title">
                <?php if($currentUser['role_id'] ==1 || $currentUser['role_id'] ==4 ){?>
                    <?php echo __('Requisition Pending For Approval'); ?> 
                <?php }?>
                <?php if($currentUser['role_id'] ==3 ){?>
                    <?php echo __('My Requisition'); ?> 
                <?php }?> 
            </h3>
        </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="actions" style="text-align: center;">  
                                    ...
                                    <?php echo $this->Html->link(__('<span class="fa fa-location-arrow"></span> Prepared delivery order'),array('action' => 'delivery', $requisition['Requisition']['id'],'received'),array('class'=>'btn btn-info btn-sm','escape' =>false)); ?>

                                    <?php }?>
                                    <?php if($currentUser['role_id'] ==3 || $currentUser['role_id'] ==4 ){?>
                                    <?php echo $this->Html->link(__('<i class="fa fa-eye" title="View Requisition"></i>'), array('action' => 'viewr', $requisition['Requisition']['id'], $requisition['Requisition']['user_id'],'received'),array('escape' =>false)); ?>
                                    <?php }?> 
                                    <?php if($currentUser['role_id'] ==1){?>
                                    <?php echo $this->Html->link(__('<i class="fa fa-eye" title="View Requisition"></i>'), array('action' => 'edit', $requisition['Requisition']['id'], $requisition['Requisition']['user_id'],'received'),array('escape' =>false)); ?>
                                    <?php }?> 
                                </td>
