...
<div class="col-sm-12 table-responsive">  
    ...
    <table class="table table-bordered more-tbl f-con-tb">
        ...
        foreach($product_datas as $product_datas) {
            ...
            $sql  = "SELECT pt.id, pt.finalcode,s.squantity, d.dquantity, p.pquantity,rr.rrquantity,rd.rdquantity,dm.dmquantity FROM products 
                AS pt LEFT JOIN
                ( 
                ...
                ( 
                    ...
                    FROM requisitionreturns GROUP BY requisitionreturns.product_id 
                )
                AS rr ON pt.id = rr.product_id LEFT JOIN
                ( 
                    SELECT requisitiondetails.product_id, SUM(requisitiondetails.quantity) AS rdquantity FROM requisitiondetails WHERE requisitiondetails.status IN (1, 2) GROUP BY requisitiondetails.product_id  
                )
                AS rd ON pt.id = rd.product_id LEFT JOIN
                ( 
                    SELECT damages.product_id,SUM(damages.quantity) AS dmquantity 
                    FROM damages GROUP BY damages.product_id 
                )
                ...
                $stockOut = ($data['dquantity']+$data['dmquantity']+$data['rdquantity']);
            ...
            <tr id="rmid<?php echo $class;?>"> 
                ...
                <td>
                    <input type="number" name="data[Requisitiondetail][<?php echo $i;?>][quantity]" id="quantity<?php echo $class;?>" onkeyup="validValue(<?php echo $class;?>,this.value,<?php echo $sval;?>);" onclick="validValue(<?php echo $class;?>,this.value,<?php echo $sval;?>);" class="form-control qnt"  required="required" value="<?php echo $balance >= 1 ? 1 : 0; ?>" min="1" max="<?php echo $balance; ?>">
                </td>

                <td><input type="text" name="data[Requisitiondetail][<?php echo $i;?>][measure_name]" id="measure_name<?php echo $class;?>"  class="form-control" value="<?php echo $product_datas['Measure']['name'];?>"> </td>
                ...
                <td><?php echo $this->Form->input("Requisitiondetail.$i.purpose",array('onchange' => "checkpurpose(this.value,$class);",'class'=>'form-horizontal','type'=>'select','options'=>$purpose,'label'=>false,'required'));?>
                        <?php echo $this->Form->input("Requisitiondetail.$i.purposeothers",array('class'=>'form-horizontal purpose','type'=>'text','rows'=>'2','label'=>false,'id'=>'purpose'.$class,'style'=>'display:none'));?> 
                </td>
    ...
    <div class="col-sm-12 p-rgt p-lft text-center"> 
        <?php if (!empty($product_datas['Product']['finalcode'])) {?> 
            <button type="submit" class="btn btn-default hdb" style="background-color:#428BCA;color:white;" onclick="return confirmSubmitRequisition();">Submit <i class="fa fa-send-o"></i></button>
        <?php }else{?> 
...
<script>
    function confirmSubmitRequisition() {
